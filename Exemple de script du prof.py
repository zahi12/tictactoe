...
if __name__ == "__main__":
    ...
    class Board(): #Le plateau de jeu entier
        def __init__(self):
            self.grids = []
            for i in range(9):
                self.grid.append(Grid()) #crée les 9 grilles
    ...
    class Grid() : #La grille 3x3
        ...
        def grid_is_full (a,b) : #teste si la grille est pleine
            pass
...
if __name__ == "__main__": 
    ...
    if board.get_case(grid=(2, 3), case=(1, 2)):
        print("a")
    if player.get_victory_great_grid() :                                  #la victoire de great grid
        grid.property = player.prenom
