...
if __name__ == "__main__": 
    ...
    for i in range (9) :            #cette boucle correspond à un affichage des cases jouées, c'est pour moi pour pouvoir suivre le déroulement du scénario
        grid=board.grids[i]
        for j in range(9):
            case=grid.cases[j]
            if case.case_is_full():
    ...
    print(board.get_grid((2,3)).owner)
    print(board.get_case((2,3),(1,1)).owner)
    
    
    if board.get_case(grid_position=(2,3), case_position=(1,1)).owner != 2:
        raise Exception("Erreur, on attendait un X dans la case 1,3 de la grille 1,3")
        
      
    if player.win_grid(grid_position = (2,3)):                                 #la victoire de grid
        raise Exception("on a pas gagné la grille")
    if player.win_game() :                                             #la victoire totale
        raise Exception("on a pas gagné la partie")        
    if player.draw_grid(grid_position = (2,3)) : 
        raise Exception("Pas égalité dans la grille")                         #l'égalité sur great grid      
    if player.draw_game() :                                              #égalité totale
        raise Exception("Pas égalité dans la partie")  
    ...
    if player.symbol != 1:                                     #on vérifie le symbole
        raise Exception("Erreur, on attendait le joueur Rond")
    ...
    if board.get_case(grid_position=(1,3), case_position=(1,1)).owner != 1:
        raise Exception("Erreur, on attendait un O dans la case 1,1 de la grille 2,3")
"""       
    if player.win_grid(grid_position = (1,3)) :                                  #la victoire de grid
        raise Exception("on a pas gagné la grille")
    if player.win_game() :                                             #la victoire totale
        raise Exception("on a pas gagné la partie")        
    ...
    if board.get_case(grid_position=(1,1), case_position=(2,2)).owner != 2:
        raise Exception("Erreur, on attendait un X dans la case 2,2 de la grille 1,1")
    ...
    if board.get_case(grid_position=(2,3), case_position=(1,3)).owner != 2:
